...
- A controllable set of **confidence/quality signals** (confidence metrics, data-quality flags, anomaly/shock indicators) that can produce:
  - normal confidence
  - degraded confidence due to missing inputs, shocks, or anomalies fileciteturn11file0
  - unavailable confidence signals (missing) fileciteturn11file0
  - false degradation signals that should be dismissed after validation fileciteturn11file0
- A controllable **Visualization Module** supporting:
  - successful rendering of the degradation indicator (banner/icon/message) fileciteturn11file0
  - injected rendering failure for the indicator fileciteturn11file0
- Observability:
  - UI states observable (indicator shown, indicator absent, forecast shown normally, error messaging if any)
  - logs accessible for assertions (detection, validation, rendering, and display events) fileciteturn11file0
...
- Confidence/quality signals are available. fileciteturn11file0
...
**Preconditions**
- Configure confidence/quality signals to reflect degraded confidence (e.g., missing recent input data, abnormal shock, unresolved anomaly). fileciteturn11file0
...
**Covers**: Main Success Scenario Step 4 fileciteturn11file0  
**Preconditions**
...
**Expected Results**
- System prepares a visual degradation indicator (e.g., banner/icon/message). fileciteturn11file0
- Indicator content communicates that uncertainty is elevated (and optionally why). fileciteturn11file0

---

## AT-05 — UI displays forecast together with the degradation indicator
**Covers**: Main Success Scenario Step 5; Success End Condition fileciteturn11file0  
**Preconditions**
...
**Expected Results**
- UI displays a clear “confidence degraded” indicator alongside the forecast. fileciteturn11file0
- Operational Manager can understand uncertainty is elevated (indicator is visible, readable, and not easily missed). fileciteturn11file0
...
**Expected Results**
- Logs include that degraded confidence was detected and displayed. fileciteturn11file0
...
**Covers**: Extension 2a (2a1–2a2) fileciteturn11file0  
**Preconditions**
- Configure confidence/quality signals retrieval to be unavailable/missing. fileciteturn11file0
...
**Expected Results**
- System logs missing confidence data. fileciteturn11file0
- Forecast is displayed without a degradation indicator (because confidence cannot be assessed). fileciteturn11file0
...
**Preconditions**
- Configure signals to initially flag degradation but set validation rules so it should be dismissed as false. fileciteturn11file0
...
**Expected Results**
- System validates and dismisses the false degradation signal. fileciteturn11file0
- Forecast is shown normally with no degradation indicator. fileciteturn11file0
...
**Covers**: Extension 4a (4a1–4a2) fileciteturn11file0  
**Preconditions**
- Degraded confidence is detected and indicator preparation succeeds.
- Force Visualization Module to fail rendering the indicator. fileciteturn11file0
...
- Indicator is not displayed. fileciteturn11file0
- Forecast may still be displayed (per use case narrative), but without the warning indicator. fileciteturn11file0
...
**Expected Results**
- Degraded confidence indicators are shown only when supported by reliable signals. fileciteturn11file0
- Missing or invalid signals do not result in misleading warnings; outcomes are logged. fileciteturn11file0
- The Operational Manager either sees a valid caution indicator or a normal forecast view, with system transparency preserved. fileciteturn11file0
...
| AT-04 | Main Success Scenario (4) fileciteturn11file0 |
| AT-05 | Main Success Scenario (5); Success End Condition fileciteturn11file0 |
| AT-06 | Main Success Scenario (6) fileciteturn11file0 |
| AT-07 | Extension 2a fileciteturn11file0 |
| AT-08 | Extension 3a fileciteturn11file0 |
| AT-09 | Extension 4a fileciteturn11file0 |
| AT-10 | Key Behavioral Theme Across All Alternatives fileciteturn11file0 |
