...
- Foundational tasks `T007`, `T008`, `T010`, `T011`, `T013`, `T014`, and `T016` can run in parallel once `T006` is in place; `T015` can follow once `T006` exists.
- For **US1**, tests `T018`-`T022` can run in parallel, repository tasks `T023`-`T025` can run in parallel, and scheduler wiring `T031` can proceed once orchestration wiring is stable.
- For **US2**, tests `T033`-`T037` can run in parallel, and repository/schema tasks `T038`-`T039` can run in parallel before service and pipeline updates.
- For **US3**, tests `T045`-`T047` can run in parallel, followed by implementation tasks `T048` and `T051` in parallel before final orchestration/query updates.
...
Task: "Add contract tests for trigger, run status, and current dataset endpoints covering authorized roles, forbidden roles, no-backdoor access, and the updated contract in backend/tests/contract/test_ingestion_api.py"
