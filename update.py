import re
with open('/Users/sahmed/Documents/capstone/311-forecast-system/frontend/src/features/demand-comparisons/components/ComparisonResultView.tsx', 'w') as f:
...
export function ComparisonResultView({ response }: { response: DemandComparisonResponse }) {
  ...
  const chartConfiguration = useMemo(() => {
    ...
    series.forEach((s) => {
      const key = `${s.seriesType.toUpperC  e()}: ${s.serviceCategory} (${s.geographyKey ?? 'All'})`;
      lineKeys.add(key);
      s.points.forEach((p) => {
