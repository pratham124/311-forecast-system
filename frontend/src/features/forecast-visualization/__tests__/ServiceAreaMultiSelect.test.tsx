...
function renderSelect(overrides: Partial<Parameters<typeof ServiceAreaMultiSelect>[0]> = {}) {
  const defaults = {
    ...
    selectedValues: ['Roads', 'Waste', 'Transit'],
    onChange: vi.fn(),
    isOpen: true,
    onOpenChange: vi.fn(),
...
describe('ServiceAreaMultiSelect', () => {
  it('shows "All service areas" when all options selected', () => {
  ...
  it('deselects an option by clicking its checkbox (filter path)', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
  ...
  it('selects an option by clicking its unchecked checkbox (add path)', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
  ...
  it('calls onChange with all options when "All" checkbox clicked', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    renderSelect({ selectedValues: ['Roads'], onChange });
    await user.click(screen.getByLabelText('All'));
    expect(onChange).toHaveBeenCalledWith(['Roads', 'Waste', 'Transit']);
  });
  ...
  it('shows "All service areas" when selectedValues is empty', () => {
    renderSelect({ selectedValues: [], isOpen: false });
    expect(screen.getByText(/all service areas/i)).toBeInTheDocument();
  });
});
